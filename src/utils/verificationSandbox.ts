...
function isPythonRunner(runner: string): boolean {
    return runner === 'python' || runner === 'python3';
}

/**
 * Pick the Docker image for a script-mode runner. Returns null if no suitable
 * image is configured — the caller returns sandbox-unavailable so the user
 * gets a clear "install X / set ENV to enable" message rather than a crash.
 */
export function pickImageForRunner(runner: string): { image: string | null; reason?: string } {
    if (isPythonRunner(runner)) {
        return { image: process.env.SECURECODE_SANDBOX_PY_IMAGE || 'python:3.11-slim' };
    }
    switch (runner) {
        case 'node':
        ...
        case 'pnpm-tsx':
        case 'yarn-tsx':
            // node:20-alpine has `node` + `npx`; tsx runs via `npx tsx` (offline
            // if already cached, otherwise fails safely — network is disabled).
            return { image: process.env.SECURECODE_SANDBOX_IMAGE || 'node:20-alpine' };
        case 'deno': {
            const img = process.env.SECURECODE_SANDBOX_DENO_IMAGE;
            return img
                ? { image: img }
...
/**
 * Build the in-container command string for a runner + script path.
 * The path is generated by us (unique suffix), so shell quoting is not
 * required for safety; we still quote defensively in case the workspace
 * path contains spaces.
 */
export function runnerInvocation(runner: string, scriptPath: string): string {
    const p = `"${scriptPath}"`;
    switch (runner) {
        case 'node':
            return `node ${p}`;
        case 'tsx':
            // npx needs no network when the package is already in the image cache.
            // In node:20-alpine tsx is NOT preinstalled, so this will fail safely
            // unless SECURECODE_SANDBOX_IMAGE provides tsx. That's acceptable: a
            // safe failure is better than silently running with `node` (which
            // would choke on TS syntax and also mask the misconfiguration).
            return `npx --no-install tsx ${p}`;
        case 'pnpm-tsx':
            return `pnpm exec tsx ${p}`;
        case 'yarn-tsx':
            return `yarn tsx ${p}`;
        case 'bun':
            return `bun run ${p}`;
        case 'deno':
            // Deno in Docker: allow-read the workspace so the test can import
            // project modules. No network (--no-remote isn't available in the
            // container deno CLI the same way; we rely on --network=none at the
            // docker level). ESM-only.
            return `deno run --allow-read=/workspace --allow-env=NODE_ENV --no-prompt ${p}`;
        case 'python':
        case 'python3':
            return `python ${p}`;
        default:
            // Unknown runner — fall back to node rather than exec'ing arbitrary
            // strings. The validator upstream should have rejected this already.
            return `node ${p}`;
    }
}
...
class DockerSandbox implements SandboxBackend {
    ...
    private async executeScript(opts: SandboxExecuteOptions): Promise<SandboxExecuteResult> {
        ...
        const ext = fileExtFor(runner);
        const testFile = path.join(sandboxDir, `verify-test-${uniqueSuffix()}${ext}`);
        const setupExt = isPythonRunner(runner) ? '.py' : ext;
        const setupFile = opts.setupScript
            ? path.join(sandboxDir, `verify-setup-${uniqueSuffix()}${setupExt}`)
            : null;
        ...
        try {
            ...
            const imageInfo = pickImageForRunner(runner);
            if (!imageInfo.image) {
                return {
                    verdict: 'sandbox-unavailable',
                    output: imageInfo.reason || `No sandbox image for runner: ${runner}`,
                    exitCode: -1,
                    backend: this.name,
                };
            }
            ...
            const testCmd = runnerInvocation(runner, containerTestPath);
            const fullCmd = setupFile
                ? `${runnerInvocation(runner, containerSetupPath)} && ${testCmd}`
                : testCmd;

            // Use sh -c so setup && test chain correctly. The script paths are
            // generated by us (unique suffix) so there is no injection risk; the
            // script *content* is already accepted-risk (it runs in the sandbox).
            dockerArgs.push('sh', '-c', fullCmd);
...
function runSandboxedProcess(
    ...
    return new Promise((resolve) => {
        ...
        proc.on('close', (code) => {
            ...
            // Cancellation takes priority over timeout — if the user aborted,
            // we must not report a timeout (which can trigger a retry).
            if (cancelled) {
                resolve({
                    verdict: 'cancelled',
                    output: `Test cancelled by user. Partial output:\n${output.slice(0, 4000)}`,
                    exitCode: -1,
                    backend: backendLabel,
                });
                return;
